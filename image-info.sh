...
sed -i "s/^PRETTY_NAME=.*/PRETTY_NAME=\"Filotimo $FEDORA_MAJOR_VERSION (FROM Fedora ${BASE_IMAGE_NAME^})\"/" /usr/lib/os-release
...
echo "BOOTLOADER_NAME=\"$IMAGE_PRETTY_NAME\"" >> /usr/lib/os-release
